(* Content-type: application/vnd.wolfram.mathematica *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 14.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       158,          7]
NotebookDataLength[     57313,       1502]
NotebookOptionsPosition[     53156,       1432]
NotebookOutlinePosition[     53652,       1450]
CellTagsIndexPosition[     53609,       1447]
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{

Cell[CellGroupData[{
Cell["\:5317\:4eac\:90ae\:7535\:5927\:5b66 2023-2024 \:5b66\:5e74\:7b2c\:4e8c\
\:5b66\:671f\[LineSeparator]\:300a\:9ad8\:7b49\:6570\:5b66A\:ff08\:4e0b\:ff09\
\:300b\:671f\:672b\:8003\:8bd5\:8bd5\:9898\:ff08A\:5377\:ff09\[LineSeparator]\
\:53c2\:8003\:7b54\:6848", "Subtitle",
 CellChangeTimes->{{3.928133447083939*^9, 3.928133477244082*^9}, {
  3.928133591336872*^9, 3.928133603315089*^9}, {3.928185151770752*^9, 
  3.928185153423068*^9}, {3.928186565770628*^9, 3.92818656757427*^9}, {
  3.928186599487554*^9, 3.928186600550672*^9}},
 TextAlignment->Center,ExpressionUUID->"8f2348b4-bb9e-470b-ab5c-4e4110300b14"],

Cell["\:5236\:4f5c\:ff1acppHusky", "Author",
 CellChangeTimes->{{3.928133608017725*^9, 3.928133610670356*^9}},
 TextAlignment->Center,ExpressionUUID->"09e2fdda-fdf0-41e9-a764-416bc5b5e69b"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:4e00\:3001\:586b\:7a7a\:9898\:ff08\:6bcf\:5c0f\:98983\:5206\:ff0c\
\:517130\:5206\:ff09", "Subtitle",
 CellChangeTimes->{{3.928133643572076*^9, 
  3.928133660533695*^9}},ExpressionUUID->"f0f5bbca-cdac-4248-9382-\
71437bff158d"],

Cell[TextData[{
 "1.  ",
 Cell[BoxData[
  FormBox[
   FractionBox["2", "3"], TraditionalForm]],ExpressionUUID->
  "a37412e9-7f67-4933-b027-2d15d4ed2a42"]
}], "Text",
 CellChangeTimes->{{3.928133667861955*^9, 
  3.928133723962123*^9}},ExpressionUUID->"ff9705ea-bdcc-4c5e-8af7-\
83df3efe42cc"],

Cell["2.  \:53d1\:6563", "Text",
 CellChangeTimes->{{3.9281336678619537`*^9, 
  3.928133761149425*^9}},ExpressionUUID->"653a5bda-b750-4739-9d4f-\
9cbce14d9ee9"],

Cell[TextData[{
 "3.  ",
 Cell[BoxData[
  FormBox[
   RowBox[{"p", ">=", "1"}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "1ec99334-1dac-4d48-a51a-95791050ce9f"]
}], "Text",
 CellChangeTimes->{{3.9281336678619537`*^9, 3.928133762534238*^9}, {
  3.9281337984818277`*^9, 
  3.928133800730013*^9}},ExpressionUUID->"99651b7a-0126-4e36-b517-\
ec0d0239a34c"],

Cell[TextData[{
 "4.  ",
 Cell[BoxData[
  FormBox[
   FractionBox["3", "2"], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "04c340ac-4ef0-430e-a369-c77e0932e855"]
}], "Text",
 CellChangeTimes->{{3.928133819506682*^9, 3.9281338209890833`*^9}, {
  3.9281338666675997`*^9, 
  3.9281338705747547`*^9}},ExpressionUUID->"1edd964e-2b0f-4229-abfa-\
e8dc36c66382"],

Cell[TextData[{
 "5.  ",
 Cell[BoxData[
  FormBox[
   RowBox[{"4", 
    SuperscriptBox["\[ExponentialE]", "4"]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "d05cebaa-e796-43a5-b744-a96de8722eba"]
}], "Text",
 CellChangeTimes->{{3.928133819506682*^9, 3.9281338209890833`*^9}, {
  3.9281338666675997`*^9, 3.928133871701694*^9}, {3.9281339237582903`*^9, 
  3.9281339259861917`*^9}},ExpressionUUID->"9fc42ab1-ef47-4fb6-80f2-\
7572edfbaa0e"],

Cell[TextData[{
 "6.  ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    FractionBox[
     RowBox[{"x", "-", "2"}], 
     RowBox[{"-", "3"}]], "=", 
    RowBox[{
     FractionBox[
      RowBox[{"y", "+", "1"}], 
      RowBox[{"-", "1"}]], "=", 
     FractionBox[
      RowBox[{"z", "-", "1"}], "5"]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "00608019-f37f-4083-9915-94b53e7a255a"]
}], "Text",
 CellChangeTimes->{{3.928133927990947*^9, 3.928133928849271*^9}, {
  3.9281340453350277`*^9, 
  3.92813406234138*^9}},ExpressionUUID->"942c3de0-b98b-42e7-bf20-\
4cf4d8a8e6d7"],

Cell["7.  0", "Text",
 CellChangeTimes->{{3.928134065335699*^9, 3.9281340662406*^9}, 
   3.928134971566009*^9},ExpressionUUID->"837e3d70-1fb2-44eb-955d-\
c02945b8824f"],

Cell[TextData[{
 "8.  ",
 Cell[BoxData[
  FormBox[
   RowBox[{"2", "\[Pi]"}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "0924d0a1-97f4-4d25-b0af-1b4c7fa0b36e"]
}], "Text",
 CellChangeTimes->{{3.928135189201715*^9, 3.928135190082265*^9}, {
  3.928135261856756*^9, 
  3.9281352625892763`*^9}},ExpressionUUID->"9ec857eb-80e1-4a48-b297-\
4b5e08cb73a8"],

Cell[TextData[{
 "9.  ",
 Cell[BoxData[
  FormBox[
   FractionBox["3", "4"], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "f82169ce-3efe-4143-818c-9f146f3968be"]
}], "Text",
 CellChangeTimes->{{3.928135265447081*^9, 3.928135266125719*^9}, {
  3.928135469845565*^9, 
  3.9281354708584623`*^9}},ExpressionUUID->"1356611b-445c-417a-97f5-\
a1055ca1f5a9"],

Cell[TextData[{
 "10.  ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"-", "1"}], "-", "\[ExponentialE]"}], TraditionalForm]],
  ExpressionUUID->"e4e3087a-367e-4c31-b8ca-062be20e413d"]
}], "Text",
 CellChangeTimes->{{3.928135474459296*^9, 3.92813547523971*^9}, {
  3.928135856091835*^9, 
  3.928135858200405*^9}},ExpressionUUID->"34cf3e26-ef32-4347-991b-\
f7b2dd52d547"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:4e8c\:3001(10\:5206)", "Subtitle",
 CellChangeTimes->{{3.928135863350387*^9, 3.92813586435744*^9}, {
  3.928190270705221*^9, 3.9281902741270523`*^9}, {3.928191232499648*^9, 
  3.928191233297674*^9}},ExpressionUUID->"56628706-b2ea-4f48-8693-\
7b5ff0c5c5c8"],

Cell[TextData[{
 "\:5168\:5fae\:5206: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[DifferentialD]", "z"}], "=", 
    RowBox[{
     RowBox[{
      RowBox[{"[", 
       RowBox[{
        SubscriptBox["f", "1"], "+", 
        RowBox[{"2", 
         SubscriptBox["f", "2"]}], "+", 
        RowBox[{
         RowBox[{"\[CurlyPhi]", "'"}], 
         RowBox[{"(", "g", ")"}], 
         SubscriptBox["g", "1"]}]}], "]"}], 
      RowBox[{"\[DifferentialD]", "x"}]}], "+", 
     RowBox[{
      RowBox[{"[", 
       RowBox[{
        RowBox[{"-", 
         SubscriptBox["f", "1"]}], "+", 
        SubscriptBox["f", "2"], "+", 
        RowBox[{
         RowBox[{"\[CurlyPhi]", "'"}], 
         RowBox[{"(", "g", ")"}], 
         SubscriptBox["g", "2"]}]}], "]"}], 
      RowBox[{"\[DifferentialD]", "y"}]}]}]}], TraditionalForm]],
  ExpressionUUID->"17380fc8-2129-4b2f-ba76-9af9f2fac2a4"],
 "."
}], "Text",
 CellChangeTimes->{{3.9281843470494437`*^9, 3.928184356438003*^9}, {
  3.928184778285145*^9, 3.928184788586421*^9}, {3.928184896562335*^9, 
  3.928184948581285*^9}, {3.928186630448814*^9, 3.928186635064841*^9}, {
  3.928186983886231*^9, 
  3.92818699217027*^9}},ExpressionUUID->"d98b29cc-b3e9-416b-9728-\
89e165f8f054"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    FractionBox[
     RowBox[{
      SuperscriptBox["\[PartialD]", "2"], "z"}], 
     RowBox[{
      RowBox[{"\[PartialD]", "x"}], 
      RowBox[{"\[PartialD]", "y"}]}]], "=", 
    RowBox[{
     RowBox[{"-", 
      SubscriptBox["f", "11"]}], "+", 
     SubscriptBox["f", "12"], "-", 
     RowBox[{"2", 
      SubscriptBox["f", "21"]}], "+", 
     RowBox[{"2", 
      SubscriptBox["f", "22"]}], "+", 
     RowBox[{
      RowBox[{"\[CurlyPhi]", "''"}], 
      RowBox[{"(", "g", ")"}], 
      SubsuperscriptBox["g", "1", "2"]}], "+", 
     RowBox[{
      RowBox[{"\[CurlyPhi]", "'"}], 
      RowBox[{"(", "g", ")"}], 
      SubscriptBox["g", "12"]}]}]}], TraditionalForm]],ExpressionUUID->
  "23f04d45-e359-457d-9cd8-b20839dc2896"],
 ".\n\:53c8\:56e0\:4e3a ",
 Cell[BoxData[
  FormBox["f", TraditionalForm]],ExpressionUUID->
  "79233125-692d-4625-983d-1e9f3036a188"],
 " \:7684\:4e8c\:9636\:504f\:5bfc\:6570\:8fde\:7eed\:ff0c\:6240\:4ee5 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["f", "12"], "=", 
    SubscriptBox["f", "21"]}], TraditionalForm]],ExpressionUUID->
  "b29dce4a-fcdd-4689-8164-bc26766f07bf"],
 ". \:56e0\:6b64, ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    FractionBox[
     RowBox[{
      SuperscriptBox["\[PartialD]", "2"], "z"}], 
     RowBox[{
      RowBox[{"\[PartialD]", "x"}], 
      RowBox[{"\[PartialD]", "y"}]}]], "=", 
    RowBox[{
     RowBox[{"-", 
      SubscriptBox["f", "11"]}], "-", 
     SubscriptBox["f", "12"], "+", 
     RowBox[{"2", 
      SubscriptBox["f", "22"]}], "+", 
     RowBox[{
      RowBox[{"\[CurlyPhi]", "''"}], 
      RowBox[{"(", "g", ")"}], 
      SubsuperscriptBox["g", "1", "2"]}], "+", 
     RowBox[{
      RowBox[{"\[CurlyPhi]", "'"}], 
      RowBox[{"(", "g", ")"}], 
      SubscriptBox["g", "12"]}]}]}], TraditionalForm]],ExpressionUUID->
  "2a8b44a7-66ab-4b2b-ad24-a40517c6b519"]
}], "Text",
 CellChangeTimes->{{3.9281843470494437`*^9, 3.928184356438003*^9}, {
  3.928184778285145*^9, 3.928184788586421*^9}, {3.928184896562335*^9, 
  3.928184941975327*^9}, {3.928185180168684*^9, 3.928185186430792*^9}, {
  3.928185273543328*^9, 3.928185286057969*^9}, {3.928185402656769*^9, 
  3.928185430826435*^9}, {3.9281855294274406`*^9, 3.928185626943791*^9}, {
  3.928186278755019*^9, 3.928186289286882*^9}, {3.9281866525814266`*^9, 
  3.928186655587962*^9}, {3.928186994007035*^9, 
  3.92818700244479*^9}},ExpressionUUID->"3ce6ec23-bef8-4c54-b421-\
500d783a7e4c"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:4e09\:3001(10\:5206)", "Subtitle",
 CellChangeTimes->{{3.928185436282009*^9, 3.928185436506649*^9}, {
  3.928190276964369*^9, 
  3.928190277941925*^9}},ExpressionUUID->"a13cd86c-38ba-4812-ab2d-\
f2cdf13b2a4e"],

Cell[TextData[{
 "\:6536\:655b\:534a\:5f84 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"R", "=", 
    RowBox[{
     RowBox[{
      SubscriptBox["\[Limit]", 
       RowBox[{"n", "->", "\[Infinity]"}]], 
      RowBox[{"\[LeftBracketingBar]", 
       FractionBox[
        RowBox[{
         SuperscriptBox[
          RowBox[{"(", 
           RowBox[{"-", "1"}], ")"}], 
          RowBox[{"n", "-", "1"}]], 
         RowBox[{"(", 
          RowBox[{"n", "+", "1"}], ")"}], "n"}], 
        RowBox[{
         SuperscriptBox[
          RowBox[{"(", 
           RowBox[{"-", "1"}], ")"}], "n"], 
         RowBox[{"(", 
          RowBox[{"n", "+", "2"}], ")"}], 
         RowBox[{"(", 
          RowBox[{"n", "+", "1"}], ")"}]}]], "\[RightBracketingBar]"}]}], "=",
      "1"}]}], TraditionalForm]],ExpressionUUID->
  "ff642b21-c0a4-4c0b-890d-8331153b607e"],
 ".\n\:5f53 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"x", "=", 
    RowBox[{"\[PlusMinus]", "1"}]}], TraditionalForm]],ExpressionUUID->
  "972f19f0-531f-429b-ac38-732ff7bdfdb1"],
 " \:65f6, ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     SubscriptBox["\[Limit]", 
      RowBox[{"n", "->", "\[Infinity]"}]], 
     RowBox[{
      SuperscriptBox[
       RowBox[{"(", 
        RowBox[{"-", "1"}], ")"}], 
       RowBox[{"n", "-", "1"}]], 
      RowBox[{"(", 
       RowBox[{"n", "+", "1"}], ")"}], "n"}]}], "=", "\[Infinity]"}], 
   TraditionalForm]],ExpressionUUID->"56bf4b22-f4be-4942-b6f6-4e8c8f9ad21b"],
 ", \:6b64\:65f6\:7ea7\:6570\:4e0d\:6536\:655b.\n\:7efc\:4e0a\:6240\:8ff0, \
\:7ea7\:6570 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    UnderoverscriptBox["\[Sum]", 
     RowBox[{"n", "=", "1"}], "\[Infinity]"], 
    RowBox[{
     SuperscriptBox[
      RowBox[{"(", 
       RowBox[{"-", "1"}], ")"}], 
      RowBox[{"n", "-", "1"}]], 
     RowBox[{"(", 
      RowBox[{"n", "+", "1"}], ")"}], 
     SuperscriptBox["nx", "n"]}]}], TraditionalForm]],ExpressionUUID->
  "d608476e-2d4a-4b49-bfc2-3468ab7bcf42"],
 " \:7684\:6536\:655b\:57df\:4e3a ",
 Cell[BoxData[
  FormBox[
   RowBox[{"(", 
    RowBox[{
     RowBox[{"-", "1"}], ",", "1"}], ")"}], TraditionalForm]],ExpressionUUID->
  "04966070-08cd-411e-bc1f-302daa14178c"],
 "."
}], "Text",
 CellChangeTimes->{{3.928185992224855*^9, 3.928186063065958*^9}, {
  3.928186235576825*^9, 3.928186332023031*^9}, {3.928186639137514*^9, 
  3.928186768296988*^9}},ExpressionUUID->"d8268408-b080-42b2-83dd-\
f55a748f138d"],

Cell[TextData[{
 "\:8bb0\:548c\:51fd\:6570\:4e3a ",
 Cell[BoxData[
  FormBox[
   RowBox[{"S", "(", "x", ")"}], TraditionalForm]],ExpressionUUID->
  "012529cc-c9f8-406c-b6ec-cf5fe89b44d8"],
 ", \:5219 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    FractionBox[
     RowBox[{"S", "(", "x", ")"}], "x"], "=", 
    RowBox[{
     RowBox[{
      UnderoverscriptBox["\[Sum]", 
       RowBox[{"n", "=", "1"}], "\[Infinity]"], 
      RowBox[{
       SuperscriptBox[
        RowBox[{"(", 
         RowBox[{"-", "1"}], ")"}], 
        RowBox[{"n", "-", "1"}]], 
       RowBox[{"(", 
        RowBox[{"n", "+", "1"}], ")"}], "n", " ", 
       SuperscriptBox["x", 
        RowBox[{"n", "-", "1"}]]}]}], "=", 
     RowBox[{
      RowBox[{
       FractionBox[
        SuperscriptBox["\[DifferentialD]", "2"], 
        RowBox[{"\[DifferentialD]", 
         SuperscriptBox["x", "2"]}]], 
       RowBox[{
        UnderoverscriptBox["\[Sum]", 
         RowBox[{"n", "=", "1"}], "\[Infinity]"], 
        SuperscriptBox[
         RowBox[{"(", 
          RowBox[{"-", "x"}], ")"}], 
         RowBox[{"n", "+", "1"}]]}]}], "=", 
      RowBox[{
       FractionBox[
        SuperscriptBox["\[DifferentialD]", "2"], 
        RowBox[{"\[DifferentialD]", 
         SuperscriptBox["x", "2"]}]], 
       FractionBox["1", 
        RowBox[{"1", "+", "x"}]]}]}]}]}], TraditionalForm]],ExpressionUUID->
  "ece57eb6-edb4-49f9-9452-82c5368a5c5e"],
 ".\n\:4e8e\:662f ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"S", "(", "x", ")"}], "=", 
    FractionBox[
     RowBox[{"2", "x"}], 
     SuperscriptBox[
      RowBox[{"(", 
       RowBox[{"x", "+", "1"}], ")"}], "3"]]}], TraditionalForm]],
  ExpressionUUID->"b203feae-bdbe-4539-a2a0-94183cd46a3a"],
 "."
}], "Text",
 CellChangeTimes->{{3.928186803397671*^9, 3.9281868177536097`*^9}, {
  3.928187044572216*^9, 3.9281870627945147`*^9}, {3.9281871240075912`*^9, 
  3.928187244657838*^9}},ExpressionUUID->"6dd12d25-0104-47a6-b7fe-\
fdc76c1c2d07"],

Cell[TextData[{
 "\:4ee3\:5165 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"x", "=", 
    RowBox[{"-", 
     FractionBox["1", "3"]}]}], TraditionalForm]],ExpressionUUID->
  "e33d42ef-1f3d-4e97-a6ae-6f343592a017"],
 " \:53ef\:4ee5\:5f97\:5230 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"S", "(", 
     RowBox[{"-", 
      FractionBox["1", "3"]}], ")"}], "=", 
    RowBox[{
     RowBox[{
      UnderoverscriptBox["\[Sum]", 
       RowBox[{"n", "=", "1"}], "\[Infinity]"], 
      FractionBox[
       RowBox[{
        RowBox[{"-", 
         RowBox[{"(", 
          RowBox[{"n", "+", "1"}], ")"}]}], "n"}], 
       SuperscriptBox["3", "n"]]}], "=", 
     RowBox[{"-", 
      FractionBox["9", "4"]}]}]}], TraditionalForm]],ExpressionUUID->
  "8dfdf9ba-0b39-4140-8466-df7dc9b8d32b"],
 ", \:6240\:4ee5 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     UnderoverscriptBox["\[Sum]", 
      RowBox[{"n", "=", "1"}], "\[Infinity]"], 
     FractionBox[
      RowBox[{
       RowBox[{"(", 
        RowBox[{"n", "+", "1"}], ")"}], "n"}], 
      SuperscriptBox["3", "n"]]}], "=", 
    FractionBox["9", "4"]}], TraditionalForm]],ExpressionUUID->
  "c8e5255d-d2b7-4b91-9d11-a343f000a28c"],
 "."
}], "Text",
 CellChangeTimes->{{3.928187245932996*^9, 3.928187270966954*^9}, {
  3.928187303255744*^9, 
  3.928187446052276*^9}},ExpressionUUID->"d6ad2dc2-b98b-4b55-b3bb-\
098bf914d557"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:56db\:3001(10\:5206)", "Subtitle",
 CellChangeTimes->{{3.928187463922789*^9, 3.928187469026635*^9}, {
  3.928187534474977*^9, 3.928187549465829*^9}, {3.928190279737709*^9, 
  3.928190281617642*^9}},ExpressionUUID->"1c9483e9-a58f-41ec-a298-\
045293d6db01"],

Cell[BoxData[
 GraphicsBox[
  TagBox[RasterBox[CompressedData["
1:
    "], {{0, 392.}, {394., 0}}, {0, 255},
    ColorFunction->RGBColor],
   BoxForm`ImageTag["Byte", ColorSpace -> "RGB", Interleaving -> True],
   Selectable->False],
  DefaultBaseStyle->"ImageGraphics",
  ImageSize->{150.25, Automatic},
  ImageSizeRaw->{394., 392.},
  PlotRange->{{0, 394.}, {0, 392.}}]], "Text",
 CellChangeTimes->{{3.928206179549141*^9, 3.928206200903981*^9}, {
  3.928209391405789*^9, 
  3.928209394454237*^9}},ExpressionUUID->"480cefaa-fb73-4811-94a3-\
dc09cc80f651"],

Cell[TextData[{
 "\:5148\:5c06\:79ef\:5206\:533a\:57df\:5206\:6210\:4e24\:90e8\:5206, \:5982\
\:56fe\:6240\:793a.\n\:56e0\:4e3a ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SuperscriptBox["x", "3"], 
    RowBox[{"ln", "(", 
     RowBox[{"y", "+", 
      SqrtBox[
       RowBox[{
        SuperscriptBox["y", "2"], "+", "1"}]]}], ")"}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "0614921c-730e-4c27-ba5e-415d4d20311b"],
 " \:662f\:5173\:4e8e ",
 Cell[BoxData[
  FormBox["x", TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "5a61b7c3-4a84-4842-a29e-4fcd8145ea32"],
 " \:548c ",
 Cell[BoxData[
  FormBox["y", TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "62c38df3-9e9b-46eb-b825-e90808e651ea"],
 " \:7684\:5947\:51fd\:6570, \:6240\:4ee5\:5bf9\:4e8e I \:533a\:57df (\:5173\
\:4e8e ",
 Cell[BoxData[
  FormBox["y", TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "0acbacad-da37-4412-b271-099e6882518b"],
 " \:8f74\:5bf9\:79f0) \:548c II \:533a\:57df (\:5173\:4e8e ",
 Cell[BoxData[
  FormBox["x", TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "c02aed51-fb98-4fb1-8e6b-6582ef2cc36b"],
 " \:8f74\:5bf9\:79f0) \:5404\:6709 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[Integral]", 
     RowBox[{"\[Integral]", 
      RowBox[{
       SuperscriptBox["x", "3"], 
       RowBox[{"ln", "(", 
        RowBox[{"y", "+", 
         SqrtBox[
          RowBox[{
           SuperscriptBox["y", "2"], "+", "1"}]]}], ")"}], 
       RowBox[{"\[DifferentialD]", "x"}], 
       RowBox[{"\[DifferentialD]", "y"}]}]}]}], "=", "0"}], TraditionalForm]],
  
  FormatType->TraditionalForm,ExpressionUUID->
  "e55c75cc-3505-48a2-8f1e-3defbec1f195"],
 "."
}], "Text",
 CellChangeTimes->{{3.9282062142975283`*^9, 
  3.92820640904667*^9}},ExpressionUUID->"5d7e30b0-9230-4175-82ae-\
197623737009"],

Cell[TextData[{
 "\:800c\:8fd9\:4e2a\:79ef\:5206\:7684\:7b2c\:4e00\:9879\:53ea\:9700\:7528\
\:6781\:5750\:6807\:5f62\:5f0f\:6c42\:89e3\:5373\:53ef. \n",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[Integral]", 
     RowBox[{"\[Integral]", 
      RowBox[{
       FractionBox["x", 
        RowBox[{"1", "+", 
         SuperscriptBox["x", "2"], "+", 
         SuperscriptBox["y", "2"]}]], 
       RowBox[{"\[DifferentialD]", "x"}], 
       RowBox[{"\[DifferentialD]", "y"}]}]}]}], "=", 
    RowBox[{
     RowBox[{"\[Integral]", 
      RowBox[{"\[Integral]", 
       RowBox[{
        FractionBox[
         RowBox[{
          SuperscriptBox["\[Rho]", "2"], "cos\[Theta]"}], 
         RowBox[{"1", "+", 
          SuperscriptBox["\[Rho]", "2"]}]], 
        RowBox[{"\[DifferentialD]", "\[Rho]"}], 
        RowBox[{"\[DifferentialD]", "\[Theta]"}]}]}]}], "=", 
     RowBox[{
      RowBox[{
       SubsuperscriptBox["\[Integral]", 
        RowBox[{"-", 
         FractionBox["\[Pi]", "4"]}], 
        FractionBox[
         RowBox[{"3", "\[Pi]"}], "4"]], 
       RowBox[{
        RowBox[{"\[DifferentialD]", "\[Theta]"}], 
        RowBox[{
         SubsuperscriptBox["\[Integral]", "1", "3"], 
         RowBox[{
          FractionBox[
           SuperscriptBox["\[Rho]", "2"], 
           RowBox[{"1", "+", 
            SuperscriptBox["\[Rho]", "2"]}]], 
          RowBox[{"\[DifferentialD]", "\[Rho]"}]}]}]}]}], "=", 
      RowBox[{"2", "-", 
       RowBox[{"arctan", "(", "3", ")"}], "+", 
       FractionBox["\[Pi]", "4"]}]}]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "299e70a0-e39c-477c-919c-fc99028a3a29"]
}], "Text",
 CellChangeTimes->{{3.928201174336567*^9, 3.928201183632291*^9}, {
  3.9282026283366823`*^9, 3.928202692222106*^9}, {3.928206414990862*^9, 
  3.928206573338202*^9}},ExpressionUUID->"8eb7e524-0626-43b8-bbdc-\
614563ece4db"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:4e94\:3001(10\:5206)", "Subtitle",
 CellChangeTimes->{{3.928187656384014*^9, 3.928187657843914*^9}, {
  3.928190283736158*^9, 
  3.928190285130487*^9}},ExpressionUUID->"497e04f0-80e0-47da-ba7a-\
df51d2c53e5b"],

Cell["\:6362\:5e8f. ", "Text",
 CellChangeTimes->{{3.9281903960429792`*^9, 3.92819052961719*^9}, {
  3.928190569069035*^9, 
  3.928190612384635*^9}},ExpressionUUID->"ae3d59f4-678e-4870-a95b-\
b5a668436622"],

Cell[TextData[Cell[BoxData[
 FormBox[
  RowBox[{"     ", 
   RowBox[{
    RowBox[{
     SubsuperscriptBox["\[Integral]", "0", "1"], 
     RowBox[{
      RowBox[{"\[DifferentialD]", "y"}], 
      RowBox[{
       SubsuperscriptBox["\[Integral]", "0", 
        RowBox[{"1", "-", "y"}]], 
       RowBox[{
        RowBox[{"\[DifferentialD]", "z"}], 
        RowBox[{
         SubsuperscriptBox["\[Integral]", "0", 
          RowBox[{"1", "-", "y", "-", "z"}]], 
         RowBox[{
          RowBox[{"(", 
           RowBox[{"1", "-", "x"}], ")"}], 
          SuperscriptBox["\[ExponentialE]", 
           RowBox[{"-", 
            SuperscriptBox[
             RowBox[{"(", 
              RowBox[{"1", "-", "x", "-", "z"}], ")"}], "2"]}]], 
          RowBox[{"\[DifferentialD]", "x"}]}]}]}]}]}]}], 
    "\[IndentingNewLine]", 
    RowBox[{"=", 
     RowBox[{
      SubsuperscriptBox["\[Integral]", "0", "1"], 
      RowBox[{
       RowBox[{"(", 
        RowBox[{"1", "-", "x"}], ")"}], 
       RowBox[{"\[DifferentialD]", "x"}], 
       RowBox[{
        SubsuperscriptBox["\[Integral]", "0", 
         RowBox[{"1", "-", "x"}]], 
        RowBox[{
         SuperscriptBox["\[ExponentialE]", 
          RowBox[{"-", 
           SuperscriptBox[
            RowBox[{"(", 
             RowBox[{"1", "-", "x", "-", "z"}], ")"}], "2"]}]], 
         RowBox[{"\[DifferentialD]", "z"}], 
         RowBox[{
          SubsuperscriptBox["\[Integral]", "0", 
           RowBox[{"1", "-", "x", "-", "z"}]], 
          RowBox[{"\[DifferentialD]", "y"}]}]}]}]}]}]}], 
    "\[IndentingNewLine]", 
    RowBox[{"=", 
     RowBox[{
      SubsuperscriptBox["\[Integral]", "0", "1"], 
      RowBox[{
       RowBox[{"(", 
        RowBox[{"1", "-", "x"}], ")"}], 
       RowBox[{"\[DifferentialD]", "x"}], 
       RowBox[{
        SubsuperscriptBox["\[Integral]", "0", 
         RowBox[{"1", "-", "x"}]], 
        RowBox[{
         RowBox[{"(", 
          RowBox[{"1", "-", "x", "-", "z"}], ")"}], 
         SuperscriptBox["\[ExponentialE]", 
          RowBox[{"-", 
           SuperscriptBox[
            RowBox[{"(", 
             RowBox[{"1", "-", "x", "-", "z"}], ")"}], "2"]}]], 
         RowBox[{"\[DifferentialD]", "z"}]}]}]}]}]}], "\[IndentingNewLine]", 
    RowBox[{"=", 
     RowBox[{
      FractionBox["1", "2"], 
      RowBox[{
       SubsuperscriptBox["\[Integral]", "0", "1"], 
       RowBox[{
        RowBox[{
         RowBox[{"(", 
          RowBox[{"1", "-", "x"}], ")"}], "[", 
         RowBox[{"1", "-", 
          SuperscriptBox["\[ExponentialE]", 
           RowBox[{"-", 
            SuperscriptBox[
             RowBox[{"(", 
              RowBox[{"1", "-", "x"}], ")"}], "2"]}]]}], "]"}], 
        RowBox[{"\[DifferentialD]", "x"}]}]}]}]}], "\[IndentingNewLine]", 
    RowBox[{"=", 
     FractionBox["1", 
      RowBox[{"4", "\[ExponentialE]"}]]}]}]}], TraditionalForm]],
 FormatType->
  TraditionalForm,ExpressionUUID->"c52fc138-e9ba-4154-acde-e5e2ce51ec27"]], \
"Text",
 CellChangeTimes->{{3.928190617456971*^9, 3.928190731404817*^9}, {
   3.928190847889612*^9, 3.928190896916155*^9}, {3.9281909365691633`*^9, 
   3.9281909366573257`*^9}, 
   3.92819102789559*^9},ExpressionUUID->"d3216129-7750-46f4-97cb-\
b2bd4b24e103"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:516d\:3001(10\:5206)", "Subtitle",
 CellChangeTimes->{{3.9281876618028584`*^9, 3.928187662864459*^9}, {
  3.928190286661762*^9, 
  3.9281902876995687`*^9}},ExpressionUUID->"12cfe42e-39dd-4bd3-9b6c-\
d5d318451001"],

Cell[TextData[{
 "\:7531\:683c\:6797\:516c\:5f0f\:53ef\:77e5, ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{
      SubscriptBox["\[ContourIntegral]", "L"], 
      RowBox[{
       RowBox[{"(", 
        RowBox[{"1", "-", 
         RowBox[{
          SuperscriptBox["x", "2"], "y"}]}], ")"}], 
       RowBox[{"\[DifferentialD]", "x"}]}]}], "+", 
     RowBox[{
      RowBox[{"(", 
       RowBox[{"1", "+", 
        RowBox[{"x", " ", 
         SuperscriptBox["y", "2"]}]}], ")"}], 
      RowBox[{"\[DifferentialD]", "y"}]}]}], "=", 
    RowBox[{"\[Integral]", 
     RowBox[{
      SubscriptBox["\[Integral]", "S"], 
      RowBox[{
       RowBox[{"(", 
        RowBox[{
         SuperscriptBox["y", "2"], "+", 
         SuperscriptBox["x", "2"]}], ")"}], 
       RowBox[{"\[DifferentialD]", "x"}], 
       RowBox[{"\[DifferentialD]", "y"}]}]}]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "0ca929b3-e3bd-41fe-92fa-6c3b9c035a29"],
 ", \:5176\:4e2d ",
 Cell[BoxData[
  FormBox["S", TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "78ded45d-514e-4d4c-87fd-23e2593b61a5"],
 " \:662f\:5706\:9762 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     SuperscriptBox["x", "2"], "+", 
     SuperscriptBox["y", "2"]}], "=", 
    RowBox[{"2", "x"}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "58424b6b-d922-4951-8d9c-23650a99352d"],
 " \:7684\:5185\:90e8."
}], "Text",
 CellChangeTimes->{{3.928187997475544*^9, 3.9281880632432423`*^9}, {
  3.928188183049683*^9, 
  3.9281882246906843`*^9}},ExpressionUUID->"82716b1d-2531-43ac-a6ca-\
e10277837f90"],

Cell[TextData[{
 "\:518d\:7528\:6781\:5750\:6807\:5f62\:5f0f\:6c42\:89e3: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[Integral]", 
     RowBox[{
      SubscriptBox["\[Integral]", "S"], 
      RowBox[{
       RowBox[{"(", 
        RowBox[{
         SuperscriptBox["y", "2"], "+", 
         SuperscriptBox["x", "2"]}], ")"}], 
       RowBox[{"\[DifferentialD]", "x"}], 
       RowBox[{"\[DifferentialD]", "y"}]}]}]}], "=", 
    RowBox[{
     RowBox[{"\[Integral]", 
      RowBox[{
       SubscriptBox["\[Integral]", 
        RowBox[{"S", "'"}]], 
       RowBox[{
        SuperscriptBox["\[Rho]", "3"], 
        RowBox[{"\[DifferentialD]", "\[Rho]"}], 
        RowBox[{"\[DifferentialD]", "\[Theta]"}]}]}]}], "=", 
     RowBox[{
      RowBox[{
       SubsuperscriptBox["\[Integral]", 
        RowBox[{"-", 
         FractionBox["\[Pi]", "2"]}], 
        FractionBox["\[Pi]", "2"]], 
       RowBox[{
        SubsuperscriptBox["\[Integral]", "0", 
         RowBox[{"2", "cos\[Theta]"}]], 
        RowBox[{
         SuperscriptBox["\[Rho]", "3"], 
         RowBox[{"\[DifferentialD]", "\[Rho]"}], 
         RowBox[{"\[DifferentialD]", "\[Theta]"}]}]}]}], "=", 
      FractionBox[
       RowBox[{"3", "\[Pi]"}], "2"]}]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "7a6ef9cc-375a-49fe-963b-d755cd4510ee"],
 "."
}], "Text",
 CellChangeTimes->{{3.928188395518297*^9, 3.928188427450898*^9}, {
   3.928188493334281*^9, 3.928188513704277*^9}, {3.928188558308516*^9, 
   3.9281885583085403`*^9}, {3.9281886050159893`*^9, 3.928188712252079*^9}, 
   3.928190299992083*^9},ExpressionUUID->"1744a486-e612-4ee2-b846-\
536e2ae2e79b"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:4e03\:3001(10\:5206)", "Subtitle",
 CellChangeTimes->{{3.92818766557767*^9, 3.928187666715149*^9}, {
  3.928190291330044*^9, 
  3.928190293197229*^9}},ExpressionUUID->"6d649ef3-28c6-484a-9e22-\
366ff769a812"],

Cell[TextData[{
 "\:6839\:636e\:5bf9\:79f0\:6027, ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[Integral]", 
      RowBox[{
       SubscriptBox["\[Integral]", "\[CapitalSigma]"], 
       RowBox[{
        RowBox[{"(", 
         RowBox[{"x", "+", 
          SuperscriptBox["x", "3"]}], ")"}], 
        RowBox[{"\[DifferentialD]", "x"}], 
        RowBox[{"\[DifferentialD]", "y"}]}]}]}], "+", 
     RowBox[{
      RowBox[{"(", 
       RowBox[{"y", "+", 
        SuperscriptBox["y", "3"]}], ")"}], 
      RowBox[{"\[DifferentialD]", "z"}], 
      RowBox[{"\[DifferentialD]", "x"}]}]}], "=", "0"}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "08323e6d-70c8-40ae-b580-d09c8a205fae"],
 "."
}], "Text",
 CellChangeTimes->{{3.92820732248759*^9, 
  3.928207363881116*^9}},ExpressionUUID->"f9b614d3-5c20-4e7b-9781-\
54b3e322de60"],

Cell[TextData[{
 "\:4e8e\:662f ",
 Cell[BoxData[
  FormBox[
   RowBox[{"I", "=", 
    RowBox[{
     RowBox[{"-", "2"}], 
     RowBox[{"\[Integral]", 
      RowBox[{
       SubscriptBox["\[Integral]", "S"], 
       RowBox[{
        SqrtBox[
         RowBox[{
          SuperscriptBox["x", "2"], "+", 
          SuperscriptBox["y", "2"]}]], 
        RowBox[{"\[DifferentialD]", "x"}], 
        RowBox[{"\[DifferentialD]", "y"}]}]}]}]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "4cbcefe8-4420-4d15-b047-1afbbfb7c216"],
 ", \:518d\:7528\:6781\:5750\:6807\:6c42\:89e3\:5f97 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"I", "=", 
    RowBox[{
     RowBox[{
      RowBox[{"-", "2"}], 
      RowBox[{
       SubsuperscriptBox["\[Integral]", "0", 
        RowBox[{"2", "\[Pi]"}]], 
       RowBox[{
        RowBox[{"\[DifferentialD]", "\[Theta]"}], 
        RowBox[{
         SubsuperscriptBox["\[Integral]", "0", "1"], 
         RowBox[{
          SuperscriptBox["\[Rho]", "2"], 
          RowBox[{"\[DifferentialD]", "\[Rho]"}]}]}]}]}]}], "=", 
     RowBox[{"-", 
      FractionBox[
       RowBox[{"4", "\[Pi]"}], "3"]}]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "76f8e21c-f35c-4782-b75f-8abce70a0ebe"],
 "."
}], "Text",
 CellChangeTimes->{{3.928207367313299*^9, 3.928207372208993*^9}, {
  3.928207442876319*^9, 
  3.9282075348279657`*^9}},ExpressionUUID->"4d6cf913-46d1-44ab-8107-\
f7a6fc25ce6c"]
}, Open  ]],

Cell[CellGroupData[{

Cell["\:516b\:3001(10\:5206) (\:7531 Proleta \:63d0\:51fa)", "Subtitle",
 CellChangeTimes->{{3.928187670461669*^9, 3.928187671257736*^9}, {
  3.928190295411577*^9, 3.928190296276767*^9}, {3.928445916462688*^9, 
  3.92844592701546*^9}},ExpressionUUID->"fe05b6c8-4d74-452a-bf50-\
035f578384e8"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[Del]", "f"}], "=", 
    RowBox[{"(", 
     RowBox[{
      RowBox[{"2", "+", "y"}], ",", 
      RowBox[{"3", "+", "x"}]}], ")"}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "6c6a4f85-fc2a-4051-a6d0-93c1a207eb1f"],
 ", \:6240\:4ee5 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[LeftBracketingBar]", 
     RowBox[{"\[LeftBracketingBar]", 
      RowBox[{"\[Del]", "f"}], "\[RightBracketingBar]"}], 
     "\[RightBracketingBar]"}], "=", 
    SqrtBox[
     RowBox[{
      SuperscriptBox[
       RowBox[{"(", 
        RowBox[{"3", "+", "x"}], ")"}], "2"], "+", 
      SuperscriptBox[
       RowBox[{"(", 
        RowBox[{"2", "+", "y"}], ")"}], "2"]}]]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "fbc5ccfc-dafb-4872-88af-6f414260b536"],
 ". \:8981\:5728\:9650\:5236\:6761\:4ef6 ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     SuperscriptBox["x", "2"], "+", 
     SuperscriptBox["y", "2"]}], "=", "52"}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "b2c1dea1-d346-499b-93d7-b746afee1ded"],
 " \:4e0b\:6c42\:51fa ",
 Cell[BoxData[
  FormBox[
   RowBox[{"\[LeftBracketingBar]", 
    RowBox[{"\[LeftBracketingBar]", 
     RowBox[{"\[Del]", "f"}], "\[RightBracketingBar]"}], 
    "\[RightBracketingBar]"}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "9b904771-1dea-410c-abd7-f33c9782582d"],
 " \:7684\:6700\:5927\:503c, \:5e94\:4f7f\:7528\:62c9\:683c\:6717\:65e5\:4e58\
\:6570\:6cd5."
}], "Text",
 CellChangeTimes->{{3.928214888344646*^9, 3.928214893872985*^9}, {
  3.9282154490060053`*^9, 3.928215457482767*^9}, {3.928215683354525*^9, 
  3.928215705750873*^9}, {3.92821577381805*^9, 
  3.928215837241206*^9}},ExpressionUUID->"dc7a8ead-43bf-49ac-b134-\
98a65b2d37f9"],

Cell[TextData[{
 "\:8003\:8651  ",
 Cell[BoxData[
  FormBox[GridBox[{
     {
      RowBox[{
       RowBox[{"g", "(", 
        RowBox[{"x", ",", "y"}], ")"}], "=", 
       RowBox[{
        SuperscriptBox[
         RowBox[{"(", 
          RowBox[{"3", "+", "x"}], ")"}], "2"], "+", 
        SuperscriptBox[
         RowBox[{"(", 
          RowBox[{"2", "+", "y"}], ")"}], "2"]}]}]},
     {
      RowBox[{
       RowBox[{"L", "(", 
        RowBox[{"x", ",", "y", ",", "\[Lambda]"}], ")"}], "=", 
       RowBox[{
        SuperscriptBox[
         RowBox[{"(", 
          RowBox[{"3", "+", "x"}], ")"}], "2"], "+", 
        SuperscriptBox[
         RowBox[{"(", 
          RowBox[{"2", "+", "y"}], ")"}], "2"], "+", 
        RowBox[{"\[Lambda]", "(", 
         RowBox[{
          SuperscriptBox["x", "2"], "+", 
          SuperscriptBox["y", "2"], "-", "52"}], ")"}]}]}]}
    }], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "3e4a8e5f-cf80-4ae7-a026-c7d14ea82395"],
 ", \:6709 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"(", GridBox[{
      {
       RowBox[{
        SubscriptBox["L", "x"], "=", 
        RowBox[{
         RowBox[{
          RowBox[{"2", 
           RowBox[{"(", 
            RowBox[{"x", "+", "3"}], ")"}]}], "+", 
          RowBox[{"2", "\[Lambda]", " ", "x"}]}], "=", "0"}]}]},
      {
       RowBox[{
        SubscriptBox["L", "y"], "=", 
        RowBox[{
         RowBox[{
          RowBox[{"2", 
           RowBox[{"(", 
            RowBox[{"y", "+", "2"}], ")"}]}], "+", 
          RowBox[{"2", "\[Lambda]", " ", "y"}]}], "=", "0"}]}]},
      {
       RowBox[{
        SubscriptBox["L", "\[Lambda]"], "=", 
        RowBox[{
         RowBox[{
          SuperscriptBox["x", "2"], "+", 
          SuperscriptBox["y", "2"], "-", "52"}], "=", "0"}]}]}
     }]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "a0b6e118-01fd-474e-8800-8b8dfc1efe28"],
 ", \:89e3\:51fa ",
 Cell[BoxData[
  FormBox[
   RowBox[{"(", GridBox[{
      {
       RowBox[{"x", "=", 
        RowBox[{"-", "6"}]}]},
      {
       RowBox[{"y", "=", 
        RowBox[{"-", "4"}]}]},
      {
       RowBox[{"\[Lambda]", "=", 
        RowBox[{"-", 
         FractionBox["1", "2"]}]}]}
     }]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "eeab9539-a720-4133-8df7-c6cdc48289be"],
 " \:6216 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"(", GridBox[{
      {
       RowBox[{"x", "=", "6"}]},
      {
       RowBox[{"y", "=", "4"}]},
      {
       RowBox[{"\[Lambda]", "=", 
        RowBox[{"-", 
         FractionBox["3", "2"]}]}]}
     }]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "8d7c29a7-3305-42bd-b42e-966b09079d26"],
 "."
}], "Text",
 CellChangeTimes->{{3.928215956747739*^9, 3.9282161141983633`*^9}, {
  3.928216148673484*^9, 
  3.92821617854152*^9}},ExpressionUUID->"fa80cb20-ad3d-4925-91f3-\
07a102b58158"],

Cell[TextData[{
 "\:7ecf\:8fc7\:9a8c\:8bc1, ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[LeftBracketingBar]", 
      RowBox[{"\[LeftBracketingBar]", 
       RowBox[{"\[Del]", 
        RowBox[{"f", "(", 
         RowBox[{
          RowBox[{"-", "6"}], ",", 
          RowBox[{"-", "4"}]}], ")"}]}], "\[RightBracketingBar]"}], 
      "\[RightBracketingBar]"}], "=", 
     SqrtBox["13"]}], ",", " ", 
    RowBox[{
     RowBox[{"\[LeftBracketingBar]", 
      RowBox[{"\[LeftBracketingBar]", 
       RowBox[{"\[Del]", 
        RowBox[{"f", "(", 
         RowBox[{"6", ",", "4"}], ")"}]}], "\[RightBracketingBar]"}], 
      "\[RightBracketingBar]"}], "=", 
     RowBox[{"3", 
      SqrtBox["13"]}]}]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "c2b26559-5992-4235-b673-26cabc89dd70"],
 ". \:6240\:4ee5 ",
 Cell[BoxData[
  FormBox[
   RowBox[{"f", "(", 
    RowBox[{"x", ",", "y"}], ")"}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "30ad25b4-246f-45fd-a1ee-2b69f4154b23"],
 " \:5728\:5706\:5468 ",
 Cell[BoxData[
  FormBox["C", TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "1d2fcf7b-b600-4bc3-9066-cd56d14a1cfc"],
 " \:4e0a\:65b9\:5411\:5bfc\:6570\:7684\:6700\:5927\:503c\:662f ",
 Cell[BoxData[
  FormBox[
   RowBox[{"3", 
    SqrtBox["13"]}], TraditionalForm]],
  FormatType->TraditionalForm,ExpressionUUID->
  "649a96cc-7705-4a80-9d48-d0ffc97813ac"],
 "."
}], "Text",
 CellChangeTimes->{{3.928216370793034*^9, 3.928216416152688*^9}, {
  3.9282165218721943`*^9, 3.9282165255523*^9}, {3.92821676403218*^9, 
  3.928216837120001*^9}},ExpressionUUID->"62c95812-b424-45ac-a747-\
457b076766f1"]
}, Open  ]]
},
WindowSize->{1257.75, 744},
WindowMargins->{{0, Automatic}, {Automatic, 0}},
Magnification:>1. Inherited,
FrontEndVersion->"14.0 for Linux x86 (64-bit) (December 12, 2023)",
StyleDefinitions->FrontEnd`FileName[{"Report"}, "StandardReport.nb", 
  CharacterEncoding -> "UTF-8"],
ExpressionUUID->"788bb185-a469-4de5-8c4f-3c9594b11fb6"
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[CellGroupData[{
Cell[580, 22, 612, 8, 97, "Subtitle",ExpressionUUID->"8f2348b4-bb9e-470b-ab5c-4e4110300b14"],
Cell[1195, 32, 189, 2, 43, "Author",ExpressionUUID->"09e2fdda-fdf0-41e9-a764-416bc5b5e69b"]
}, Open  ]],
Cell[CellGroupData[{
Cell[1421, 39, 235, 4, 33, "Subtitle",ExpressionUUID->"f0f5bbca-cdac-4248-9382-71437bff158d"],
Cell[1659, 45, 291, 9, 37, "Text",ExpressionUUID->"ff9705ea-bdcc-4c5e-8af7-83df3efe42cc"],
Cell[1953, 56, 160, 3, 34, "Text",ExpressionUUID->"653a5bda-b750-4739-9d4f-9cbce14d9ee9"],
Cell[2116, 61, 378, 11, 34, "Text",ExpressionUUID->"99651b7a-0126-4e36-b517-ec0d0239a34c"],
Cell[2497, 74, 377, 11, 37, "Text",ExpressionUUID->"1edd964e-2b0f-4229-abfa-e8dc36c66382"],
Cell[2877, 87, 462, 12, 34, "Text",ExpressionUUID->"9fc42ab1-ef47-4fb6-80f2-7572edfbaa0e"],
Cell[3342, 101, 593, 20, 37, "Text",ExpressionUUID->"942c3de0-b98b-42e7-bf20-4cf4d8a8e6d7"],
Cell[3938, 123, 168, 3, 34, "Text",ExpressionUUID->"837e3d70-1fb2-44eb-955d-c02945b8824f"],
Cell[4109, 128, 374, 11, 34, "Text",ExpressionUUID->"9ec857eb-80e1-4a48-b297-4b5e08cb73a8"],
Cell[4486, 141, 373, 11, 37, "Text",ExpressionUUID->"1356611b-445c-417a-97f5-a1055ca1f5a9"],
Cell[4862, 154, 376, 11, 34, "Text",ExpressionUUID->"34cf3e26-ef32-4347-991b-f7b2dd52d547"]
}, Open  ]],
Cell[CellGroupData[{
Cell[5275, 170, 264, 4, 33, "Subtitle",ExpressionUUID->"56628706-b2ea-4f48-8693-7b5ff0c5c5c8"],
Cell[5542, 176, 1223, 37, 37, "Text",ExpressionUUID->"d98b29cc-b3e9-416b-9728-89e165f8f054"],
Cell[6768, 215, 2478, 72, 76, "Text",ExpressionUUID->"3ce6ec23-bef8-4c54-b421-500d783a7e4c"]
}, Open  ]],
Cell[CellGroupData[{
Cell[9283, 292, 217, 4, 33, "Subtitle",ExpressionUUID->"a13cd86c-38ba-4812-ab2d-f2cdf13b2a4e"],
Cell[9503, 298, 2416, 77, 91, "Text",ExpressionUUID->"d8268408-b080-42b2-83dd-f55a748f138d"],
Cell[11922, 377, 1961, 62, 73, "Text",ExpressionUUID->"6dd12d25-0104-47a6-b7fe-fdc76c1c2d07"],
Cell[13886, 441, 1378, 47, 41, "Text",ExpressionUUID->"d6ad2dc2-b98b-4b55-b3bb-098bf914d557"]
}, Open  ]],
Cell[CellGroupData[{
Cell[15301, 493, 263, 4, 33, "Subtitle",ExpressionUUID->"1c9483e9-a58f-41ec-a298-045293d6db01"],
Cell[15567, 499, 17249, 288, 163, "Text",ExpressionUUID->"480cefaa-fb73-4811-94a3-dc09cc80f651"],
Cell[32819, 789, 1883, 57, 62, "Text",ExpressionUUID->"5d7e30b0-9230-4175-82ae-197623737009"],
Cell[34705, 848, 1875, 52, 73, "Text",ExpressionUUID->"8eb7e524-0626-43b8-bbdc-614563ece4db"]
}, Open  ]],
Cell[CellGroupData[{
Cell[36617, 905, 217, 4, 33, "Subtitle",ExpressionUUID->"497e04f0-80e0-47da-ba7a-df51d2c53e5b"],
Cell[36837, 911, 206, 4, 34, "Text",ExpressionUUID->"ae3d59f4-678e-4870-a95b-b5a668436622"],
Cell[37046, 917, 3214, 93, 153, "Text",ExpressionUUID->"d3216129-7750-46f4-97cb-b2bd4b24e103"]
}, Open  ]],
Cell[CellGroupData[{
Cell[40297, 1015, 221, 4, 33, "Subtitle",ExpressionUUID->"12cfe42e-39dd-4bd3-9b6c-d5d318451001"],
Cell[40521, 1021, 1633, 52, 37, "Text",ExpressionUUID->"82716b1d-2531-43ac-a6ca-e10277837f90"],
Cell[42157, 1075, 1653, 47, 48, "Text",ExpressionUUID->"1744a486-e612-4ee2-b846-536e2ae2e79b"]
}, Open  ]],
Cell[CellGroupData[{
Cell[43847, 1127, 216, 4, 33, "Subtitle",ExpressionUUID->"6d649ef3-28c6-484a-9e22-366ff769a812"],
Cell[44066, 1133, 868, 27, 35, "Text",ExpressionUUID->"f9b614d3-5c20-4e7b-9781-54b3e322de60"],
Cell[44937, 1162, 1446, 46, 41, "Text",ExpressionUUID->"4d6cf913-46d1-44ab-8107-f7a6fc25ce6c"]
}, Open  ]],
Cell[CellGroupData[{
Cell[46420, 1213, 292, 4, 33, "Subtitle",ExpressionUUID->"fe05b6c8-4d74-452a-bf50-035f578384e8"],
Cell[46715, 1219, 1849, 54, 37, "Text",ExpressionUUID->"dc7a8ead-43bf-49ac-b134-98a65b2d37f9"],
Cell[48567, 1275, 2886, 102, 82, "Text",ExpressionUUID->"fa80cb20-ad3d-4925-91f3-07a102b58158"],
Cell[51456, 1379, 1684, 50, 37, "Text",ExpressionUUID->"62c95812-b424-45ac-a747-457b076766f1"]
}, Open  ]]
}
]
*)

